...
var NGCC_VERSION = "13.0.3+17.sha-86843f8.with-local-changes";
...
// bazel-out/k8-fastbuild/bin/packages/compiler-cli/ngcc/src/packages/bundle_program.mjs
function makeBundleProgram(fs3, isCore, pkg, path2, r3FileName, options2, host, additionalFiles = []) {
  ...
  return { program, options: options2, host, package: pkg, path: path2, file, r3SymbolsPath, r3SymbolsFile };
}
...
var NgccSourcesCompilerHost = class extends NgtscCompilerHost {
  constructor(fs3, options2, cache, moduleResolutionCache, packagePath) {
    super(fs3, options2);
    this.cache = cache;
...
var NgccDtsCompilerHost = class extends NgtscCompilerHost {
  constructor(fs3, options2, cache, moduleResolutionCache) {
    super(fs3, options2);
    this.cache = cache;
    this.moduleResolutionCache = moduleResolutionCache;
  }
...
function makeEntryPointBundle(fs3, entryPoint, sharedFileCache, moduleResolutionCache, formatPath, isCore, format, dtsProcessing, pathMappings, mirrorDtsFromSrc = false, enableI18nLegacyMessageIdFormat = true) {
  ...
  const dtsHost = new NgccDtsCompilerHost(fs3, options2, entryPointCache, moduleResolutionCache);
  const srcHost = new NgccSourcesCompilerHost(fs3, options2, entryPointCache, moduleResolutionCache, entryPoint.packagePath);
  ...
  const src = makeBundleProgram(fs3, isCore, entryPoint.packagePath, absFormatPath, "r3_symbols.js", options2, srcHost);
  ...
  const dts = dtsProcessing !== DtsProcessing.No ? makeBundleProgram(fs3, isCore, entryPoint.packagePath, typingsPath, "r3_symbols.d.ts", __spreadProps(__spreadValues({}, options2), { allowJs: false }), dtsHost, additionalDtsFiles) : null;
...
function readConfiguration(project, existingOptions, host = getFileSystem()) {
  ...
  try {
    ...
    const { options: options2, errors, fileNames: rootNames, projectReferences } = ts95.parseJsonConfigFileContent(config, parseConfigHost, basePath, existingCompilerOptions, configFileName);
...
function parseCommandLineOptions(args) {
  ...
  setFileSystem(fs3);
  const baseSourcePath = fs3.resolve(options2.s || "./node_modules");
