...
var DOC_PAGE_BASE_URL = (() => {
  ...
  const isPreRelease = full.includes("-next") || full.includes("-rc") || full === "22.0.0-next.1+sha-1969338";
...
function makeTemplateDiagnostic(id, mapping, span, category, code, messageText, relatedMessages, deprecatedDiagInfo) {
  if (mapping.type === "direct") {
    ...
    let typeForMessage;
    if (category === ts42.DiagnosticCategory.Warning) {
      typeForMessage = "Warning";
    } else if (category === ts42.DiagnosticCategory.Suggestion) {
      typeForMessage = "Suggestion";
    } else if (category === ts42.DiagnosticCategory.Message) {
...
function parseTemplateAsSourceFile(fileName, template) {
  ...
  return ts42.createSourceFile(
    fileName,
    template,
    ts42.ScriptTarget.Latest,
    /* setParentNodes */
    false,
    ts42.ScriptKind.JSX
  );
...
`;
      ...
      const diag = makeTemplateDiagnostic(id, mapping, sourceSpanForDiagnostics, ts43.DiagnosticCategory.Error, ngErrorCode(ErrorCode.SCHEMA_INVALID_ELEMENT), errorMsg);
...
3. To allow any property add 'NO_ERRORS_SCHEMA' to the ${schemas2} of this component.`;
      ...
      const diag = makeTemplateDiagnostic(id, mapping, span, ts43.DiagnosticCategory.Error, ngErrorCode(ErrorCode.SCHEMA_INVALID_ATTRIBUTE), errorMsg);
      this._diagnostics.push(diag);
    }
  }
  checkHostElementProperty(id, element, name, span, schemas) {
    for (const tagName of element.tagNames) {
      ...
      const diag = makeTemplateDiagnostic(id, mapping, span, ts43.DiagnosticCategory.Error, ngErrorCode(ErrorCode.SCHEMA_INVALID_ATTRIBUTE), errorMessage);
      this._diagnostics.push(diag);
      break;
    }
  }
...
function tempPrint(node, sourceFile) {
  tempPrinter ??= ts45.createPrinter();
  return tempPrinter.printNode(ts45.EmitHint.Unspecified, node, sourceFile);
}
...
var ReferenceEmitEnvironment = class {
  ...
  referenceExternalSymbol(moduleName, name) {
    ...
    if (ts46.isIdentifier(importResult)) {
      return new TcbExpr(importResult.text);
    } else if (ts46.isIdentifier(importResult.expression)) {
...
var TypeParameterEmitter = class {
  ...
  emit(emitReference) {
    ...
    return this.typeParameters.map((typeParam) => {
      ...
      return ts47.factory.updateTypeParameterDeclaration(typeParam, typeParam.modifiers, typeParam.name, constraint, defaultType);
    });
  }
  resolveTypeReference(type) {
    const target = ts47.isIdentifier(type.typeName) ? type.typeName : type.typeName.right;
    const declaration = this.reflector.getDeclarationOfIdentifier(target);
  ...
  translateTypeReference(type, emitReference) {
    ...
    if (!ts47.isTypeReferenceNode(typeNode)) {
      throw new Error(`Expected TypeReferenceNode for emitted reference, got ${ts47.SyntaxKind[typeNode.kind]}.`);
    }
    return typeNode;
  }
...
function createHostElement(type, selector, sourceNode, literal4, bindingDecorators, listenerDecorators) {
  ...
  if (literal4 !== null) {
    for (const prop of literal4.properties) {
      if (ts48.isPropertyAssignment(prop) && ts48.isStringLiteralLike(prop.initializer) && isStaticName(prop.name)) {
...
function isHostBindingsBlockGuard(node) {
  if (!ts48.isIfStatement(node)) {
    return false;
  }
  const expr = node.expression;
  if (!ts48.isParenthesizedExpression(expr) || expr.expression.kind !== ts48.SyntaxKind.TrueKeyword) {
    return false;
  }
  const text = expr.getSourceFile().text;
  return ts48.forEachTrailingCommentRange(text, expr.expression.getEnd(), (pos, end, kind) => kind === ts48.SyntaxKind.MultiLineCommentTrivia && text.substring(pos + 2, end - 2) === GUARD_COMMENT_TEXT) || false;
}
...
function createNodeFromBindingDecorator(decorator, bindings) {
  if (!ts48.isCallExpression(decorator.expression)) {
    return;
  }
  ...
  if (property && ts48.isPropertyDeclaration(property) && isStaticName(property.name)) {
    propertyName = property.name;
  }
  if (args.length === 0) {
    nameNode = propertyName;
  } else if (ts48.isStringLiteralLike(args[0])) {
    nameNode = args[0];
  } else {
    return;
  }
  ...
  const read = ts48.isIdentifier(propertyName) ? new PropertyRead(span, nameSpan, nameSpan, receiver, propertyName.text) : new KeyedRead(span, nameSpan, receiver, new LiteralPrimitive(span, nameSpan, propertyName.text));
...
function createNodeFromListenerDecorator(decorator, parser, listeners) {
  if (!ts48.isCallExpression(decorator.expression) || decorator.expression.arguments.length === 0) {
    return;
  }
  const args = decorator.expression.arguments;
  const method = decorator.parent;
  if (!method || !ts48.isMethodDeclaration(method) || !isStaticName(method.name) || !ts48.isStringLiteralLike(args[0])) {
    return;
  }
  const span = new ParseSpan(-1, -1);
  const argNodes = [];
  ...
  const receiver = ts48.isIdentifier(method.name) ? new PropertyRead(span, nameSpan, nameSpan, methodReceiver, method.name.text) : new KeyedRead(span, nameSpan, methodReceiver, new LiteralPrimitive(span, nameSpan, method.name.text));
  if (args.length > 1 && ts48.isArrayLiteralExpression(args[1])) {
    for (const expr of args[1].elements) {
      if (ts48.isStringLiteralLike(expr)) {
...
function isStaticName(node) {
  return ts48.isIdentifier(node) || ts48.isStringLiteralLike(node);
}
function createStaticExpressionSpan(node) {
  const span = createSourceSpan(node);
  if (ts48.isStringLiteralLike(node)) {
...
function isInHostBindingTcb(node) {
  let current = node;
  while (current && !ts49.isFunctionDeclaration(current)) {
...
function findTypeCheckBlock(file, id, isDiagnosticRequest) {
  for (const stmt of file.statements) {
    if (ts49.isFunctionDeclaration(stmt) && getTypeCheckId2(stmt, file, isDiagnosticRequest) === id) {
      return stmt;
    }
  }
  return findNodeInFile(file, (node) => ts49.isFunctionDeclaration(node) && getTypeCheckId2(node, file, isDiagnosticRequest) === id);
}
function findSourceLocation(node, sourceFile, isDiagnosticsRequest) {
  while (node !== void 0 && !ts49.isFunctionDeclaration(node)) {
...
function getTypeCheckId2(node, sourceFile, isDiagnosticRequest) {
  while (!ts49.isFunctionDeclaration(node)) {
  ...
  return ts49.forEachLeadingCommentRange(sourceFile.text, start, (pos, end, kind) => {
    if (kind !== ts49.SyntaxKind.MultiLineCommentTrivia) {
      return null;
    }
    const commentText = sourceFile.text.substring(pos + 2, end - 2);
    return commentText;
...
function findNodeInFile(file, predicate) {
  const visit2 = (node) => {
    ...
    return ts49.forEachChild(node, visit2) ?? null;
  };
  return ts49.forEachChild(file, visit2) ?? null;
}
...
var TcbControlFlowContentProjectionOp = class extends TcbOp {
  ...
  constructor(tcb, element, ngContentSelectors, componentName) {
    ...
    this.category = tcb.env.config.controlFlowPreventingContentProjection === "error" ? ts50.DiagnosticCategory.Error : ts50.DiagnosticCategory.Warning;
  }
  optional = false;
  execute() {
...
function extractTemplate(node, template, evaluator, depTracker, resourceLoader, options, compilationMode) {
  if (template.isInline) {
    ...
    let sourceMapUrl;
    if (ts51.isStringLiteral(template.expression) || ts51.isNoSubstitutionTemplateLiteral(template.expression)) {
...
function getTemplateRange(templateExpr) {
  ...
  const { line, character } = ts51.getLineAndCharacterOfPosition(templateExpr.getSourceFile(), startPos);
  return {
    startPos,
    startLine: line,
    startCol: character,
...
function transformDecoratorResources(dec, component, styles, template) {
  ...
  if (metadata.has("templateUrl")) {
    ...
    metadata.set("template", ts51.factory.createStringLiteral(template.content));
  }
    ...
    if (styles.length > 0) {
      const styleNodes = styles.reduce((result, style) => {
        if (style.trim().length > 0) {
          result.push(ts51.factory.createStringLiteral(style));
        }
        return result;
      }, []);
      if (styleNodes.length > 0) {
        metadata.set("styles", ts51.factory.createArrayLiteralExpression(styleNodes));
      }
    }
  }
  const newMetadataFields = [];
  for (const [name, value] of metadata.entries()) {
    newMetadataFields.push(ts51.factory.createPropertyAssignment(name, value));
  }
  return { ...dec, args: [ts51.factory.createObjectLiteralExpression(newMetadataFields)] };
}
...
function extractStyleUrlsFromExpression(evaluator, styleUrlsExpr) {
  const styleUrls = [];
  if (ts51.isArrayLiteralExpression(styleUrlsExpr)) {
    for (const styleUrlExpr of styleUrlsExpr.elements) {
      if (ts51.isSpreadElement(styleUrlExpr)) {
...
function extractInlineStyleResources(component) {
  ...
  function stringLiteralElements(array) {
    return array.elements.filter((e) => ts51.isStringLiteralLike(e));
  }
  const stylesExpr = component.get("styles");
  if (stylesExpr !== void 0) {
    if (ts51.isArrayLiteralExpression(stylesExpr)) {
      for (const expression of stringLiteralElements(stylesExpr)) {
        styles.add({ path: null, node: expression });
      }
    } else if (ts51.isStringLiteralLike(stylesExpr)) {
...
function getSymbolIdentifier(decl) {
  if (!ts52.isSourceFile(decl.parent)) {
    return null;
  }
...
function extractSemanticTypeParameters(node) {
  if (!ts53.isClassDeclaration(node) || node.typeParameters === void 0) {
    return null;
  }
...
var LocalModuleScopeRegistry = class {
  ...
  getExportedScope(ref, diagnostics, ownerForErrors, type) {
    if (ref.node.getSourceFile().isDeclarationFile) {
      if (!ts54.isClassDeclaration(ref.node)) {
...
var SelectorlessComponentScopeReader = class {
  ...
  getAvailableIdentifiers(node) {
    const result = /* @__PURE__ */ new Map();
    let current = ts55.getOriginalNode(node).parent;
    while (current) {
      if (!ts55.isSourceFile(current) && !ts55.isBlock(current)) {
        current = current.parent;
        continue;
      }
      for (const stmt of current.statements) {
        ...
        if (ts55.isImportDeclaration(stmt) && stmt.importClause !== void 0 && !(stmt.importClause.phaseModifier === ts55.SyntaxKind.TypeKeyword)) {
          const clause = stmt.importClause;
          if (clause.namedBindings !== void 0 && ts55.isNamedImports(clause.namedBindings)) {
...
function isDirectiveDeclaration(node) {
  const sourceFile = node.getSourceFile();
  return (ts56.isTypeNode(node) || ts56.isIdentifier(node)) && ts56.isVariableDeclaration(node.parent) && hasExpressionIdentifier(sourceFile, node, ExpressionIdentifier.DIRECTIVE);
}
function isSymbolAliasOf(firstSymbol, lastSymbol, typeChecker) {
  let currentSymbol = lastSymbol;
  const seenSymbol = /* @__PURE__ */ new Set();
  while (firstSymbol !== currentSymbol && currentSymbol !== void 0 && currentSymbol.flags & ts56.SymbolFlags.Alias) {
...
var TsCreateProgramDriver = class {
  ...
  updateFiles(contents, updateMode) {
    ...
    for (const [filePath, { newText, originalFile }] of contents.entries()) {
      const sf = ts57.createSourceFile(filePath, newText, ts57.ScriptTarget.Latest, true);
    ...
    this.program = ts57.createProgram({
      host,
...
var CompletionEngine = class {
  ...
  constructor(tcb, data, tcbPath, tcbIsShim) {
    ...
    const globalRead = findFirstMatchingNode(this.tcb, {
      filter: ts58.isPropertyAccessExpression,
  ...
  getExpressionCompletionLocation(expr) {
    ...
    if (expr instanceof PropertyRead7) {
      ...
      const ternaryExpr = findFirstMatchingNode(this.tcb, {
        filter: ts58.isParenthesizedExpression,
        withSpan: expr.sourceSpan
      });
      if (ternaryExpr === null || !ts58.isConditionalExpression(ternaryExpr.expression)) {
        return null;
      }
      const whenTrue = ternaryExpr.expression.whenTrue;
      if (ts58.isPropertyAccessExpression(whenTrue)) {
        tsExpr = whenTrue;
      } else if (ts58.isCallExpression(whenTrue) && ts58.isPropertyAccessExpression(whenTrue.expression)) {
  ...
  getLiteralCompletionLocation(expr) {
    ...
    if (expr instanceof TmplAstTextAttribute) {
      ...
      if (strNode !== null && ts58.isStringLiteral(strNode.expression)) {
        tsExpr = strNode.expression;
      }
    } else {
      tsExpr = findFirstMatchingNode(this.tcb, {
        filter: (n2) => ts58.isStringLiteral(n2) || ts58.isNumericLiteral(n2),
        withSpan: expr.sourceSpan
      });
    }
    if (tsExpr === null) {
    ...
    if (ts58.isStringLiteral(tsExpr)) {
...
function adaptTypeCheckBlockMetadata(ref, meta, env) {
  ...
  const convertDir = (dir) => {
    ...
    const tcbDir = {
      ...
      hasNgFieldDirective: ts59.isClassDeclaration(dir.ref.node) && dir.ref.node.members.some((member) => ts59.isPropertyDeclaration(member) && ts59.isComputedPropertyName(member.name) && ts59.isIdentifier(member.name.expression) && member.name.expression.text === "\u0275NgFieldDirective"),
      coercedInputFields: dir.coercedInputFields,
...
function extractNameFromExpr(node) {
  if (ts59.isIdentifier(node)) {
    return node.text;
  } else if (ts59.isPropertyAccessExpression(node)) {
...
var TypeCheckFile = class extends Environment {
  ...
  addTypeCheckBlock(ref, meta, domSchemaChecker, oobRecorder, genericContextBehavior) {
    const fnId = ts62.factory.createIdentifier(`_tcb${this.nextTcbId++}`);
...
var InlineTcbOp = class {
  ...
  execute(im, sf, refEmitter) {
    ...
    const fnName = ts63.factory.createIdentifier(`_tcb_${this.ref.node.pos}`);
...
function unwrapSignalInputWriteTAccessor(expr) {
  ...
  if (!ts65.isIdentifier(expr.argumentExpression.name) || expr.argumentExpression.name.text !== R3Identifiers6.InputSignalBrandWriteType.name) {
    return null;
  }
...
function validateAndFlattenComponentImports(imports, expr, isDeferred) {
  ...
  for (let i = 0; i < imports.length; i++) {
    const ref = imports[i];
    let refExpr = expr;
    if (ts70.isArrayLiteralExpression(expr) && expr.elements.length === imports.length && !expr.elements.some(ts70.isSpreadAssignment)) {
...
function getTopLevelDeclarationNames(sourceFile) {
  ...
  for (const node of sourceFile.statements) {
    ...
    if (ts71.isImportDeclaration(node) && node.importClause) {
      ...
      if (importClause.phaseModifier === ts71.SyntaxKind.TypeKeyword) {
        continue;
      }
