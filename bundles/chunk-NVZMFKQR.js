...
var StaticInterpreter = class {
  ...
  BINARY_OPERATORS = /* @__PURE__ */ new Map([
    [ts3.SyntaxKind.PlusToken, literalBinaryOp((a, b) => a + b)],
    [ts3.SyntaxKind.MinusToken, literalBinaryOp((a, b) => a - b)],
    [ts3.SyntaxKind.AsteriskToken, literalBinaryOp((a, b) => a * b)],
    [ts3.SyntaxKind.SlashToken, literalBinaryOp((a, b) => a / b)],
    [ts3.SyntaxKind.PercentToken, literalBinaryOp((a, b) => a % b)],
    [ts3.SyntaxKind.AmpersandToken, literalBinaryOp((a, b) => a & b)],
    [ts3.SyntaxKind.BarToken, literalBinaryOp((a, b) => a | b)],
    [ts3.SyntaxKind.CaretToken, literalBinaryOp((a, b) => a ^ b)],
    [ts3.SyntaxKind.LessThanToken, literalBinaryOp((a, b) => a < b)],
    [ts3.SyntaxKind.LessThanEqualsToken, literalBinaryOp((a, b) => a <= b)],
    [ts3.SyntaxKind.GreaterThanToken, literalBinaryOp((a, b) => a > b)],
    [ts3.SyntaxKind.GreaterThanEqualsToken, literalBinaryOp((a, b) => a >= b)],
    [ts3.SyntaxKind.EqualsEqualsToken, literalBinaryOp((a, b) => a == b)],
    [ts3.SyntaxKind.EqualsEqualsEqualsToken, literalBinaryOp((a, b) => a === b)],
    [ts3.SyntaxKind.ExclamationEqualsToken, literalBinaryOp((a, b) => a != b)],
    [ts3.SyntaxKind.ExclamationEqualsEqualsToken, literalBinaryOp((a, b) => a !== b)],
    [ts3.SyntaxKind.LessThanLessThanToken, literalBinaryOp((a, b) => a << b)],
    [ts3.SyntaxKind.GreaterThanGreaterThanToken, literalBinaryOp((a, b) => a >> b)],
    [ts3.SyntaxKind.GreaterThanGreaterThanGreaterThanToken, literalBinaryOp((a, b) => a >>> b)],
    [ts3.SyntaxKind.AsteriskAsteriskToken, literalBinaryOp((a, b) => Math.pow(a, b))],
    [ts3.SyntaxKind.AmpersandAmpersandToken, referenceBinaryOp((a, b) => a && b)],
    [ts3.SyntaxKind.BarBarToken, referenceBinaryOp((a, b) => a || b)]
  ]);
  UNARY_OPERATORS = /* @__PURE__ */ new Map([
    [ts3.SyntaxKind.TildeToken, (a) => ~a],
    [ts3.SyntaxKind.MinusToken, (a) => -a],
    [ts3.SyntaxKind.PlusToken, (a) => +a],
    [ts3.SyntaxKind.ExclamationToken, (a) => !a]
  ]);
  ...
  visitExpression(node, context) {
    ...
    if (node.kind === ts3.SyntaxKind.TrueKeyword) {
      return true;
    } else if (node.kind === ts3.SyntaxKind.FalseKeyword) {
      return false;
    } else if (node.kind === ts3.SyntaxKind.NullKeyword) {
      return null;
    } else if (ts3.isStringLiteral(node)) {
      return node.text;
    } else if (ts3.isNoSubstitutionTemplateLiteral(node)) {
      return node.text;
    } else if (ts3.isTemplateExpression(node)) {
      result = this.visitTemplateExpression(node, context);
    } else if (ts3.isNumericLiteral(node)) {
      return parseFloat(node.text);
    } else if (ts3.isObjectLiteralExpression(node)) {
      result = this.visitObjectLiteralExpression(node, context);
    } else if (ts3.isIdentifier(node)) {
      result = this.visitIdentifier(node, context);
    } else if (ts3.isPropertyAccessExpression(node)) {
      result = this.visitPropertyAccessExpression(node, context);
    } else if (ts3.isCallExpression(node)) {
      result = this.visitCallExpression(node, context);
    } else if (ts3.isConditionalExpression(node)) {
      result = this.visitConditionalExpression(node, context);
    } else if (ts3.isPrefixUnaryExpression(node)) {
      result = this.visitPrefixUnaryExpression(node, context);
    } else if (ts3.isBinaryExpression(node)) {
      result = this.visitBinaryExpression(node, context);
    } else if (ts3.isArrayLiteralExpression(node)) {
      result = this.visitArrayLiteralExpression(node, context);
    } else if (ts3.isParenthesizedExpression(node)) {
      result = this.visitParenthesizedExpression(node, context);
    } else if (ts3.isElementAccessExpression(node)) {
      result = this.visitElementAccessExpression(node, context);
    } else if (ts3.isAsExpression(node)) {
      result = this.visitExpression(node.expression, context);
    } else if (ts3.isNonNullExpression(node)) {
      result = this.visitExpression(node.expression, context);
  ...
  visitIdentifier(node, context) {
    ...
    if (decl === null) {
      if (ts3.identifierToKeywordKind(node) === ts3.SyntaxKind.UndefinedKeyword) {
        return void 0;
      } else {
  ...
  visitDeclaration(node, context) {
    ...
    } else if (ts3.isParameter(node) && context.scope.has(node)) {
      return context.scope.get(node);
    } else if (ts3.isExportAssignment(node)) {
      return this.visitExpression(node.expression, context);
    } else if (ts3.isEnumDeclaration(node)) {
      return this.visitEnumDeclaration(node, context);
    } else if (ts3.isSourceFile(node)) {
      return this.visitSourceFile(node, context);
    } else if (ts3.isBindingElement(node)) {
  ...
  visitFunctionBody(node, fn, context) {
    if (fn.body === null) {
      return DynamicValue.fromUnknown(node);
    } else if (fn.body.length !== 1 || !ts3.isReturnStatement(fn.body[0])) {
  ...
  visitBindingElement(node, context) {
    ...
    while (ts3.isBindingElement(closestDeclaration) || ts3.isArrayBindingPattern(closestDeclaration) || ts3.isObjectBindingPattern(closestDeclaration)) {
      if (ts3.isBindingElement(closestDeclaration)) {
        path.unshift(closestDeclaration);
      }
      closestDeclaration = closestDeclaration.parent;
    }
    if (!ts3.isVariableDeclaration(closestDeclaration) || closestDeclaration.initializer === void 0) {
      return DynamicValue.fromUnknown(node);
    }
    let value = this.visit(closestDeclaration.initializer, context);
    for (const element of path) {
      let key;
      if (ts3.isArrayBindingPattern(element.parent)) {
  ...
  stringNameFromPropertyName(node, context) {
    if (ts3.isIdentifier(node) || ts3.isStringLiteral(node) || ts3.isNumericLiteral(node)) {
      return node.text;
    } else if (ts3.isComputedPropertyName(node)) {
  ...
  visitType(node, context) {
    ...
    } else if (ts3.isTupleTypeNode(node)) {
      return this.visitTupleType(node, context);
    } else if (ts3.isNamedTupleMember(node)) {
      return this.visitType(node.type, context);
    } else if (ts3.isTypeOperatorNode(node) && node.operator === ts3.SyntaxKind.ReadonlyKeyword) {
      return this.visitType(node.type, context);
    } else if (ts3.isTypeQueryNode(node)) {
  ...
  visitTypeQuery(node, context) {
    const exprName = ts3.isQualifiedName(node.exprName) ? node.exprName.right : node.exprName;
    if (!ts3.isIdentifier(exprName)) {
      return DynamicValue.fromUnknown(node);
    }
...
function isFunctionOrMethodReference(ref) {
  return ts3.isFunctionDeclaration(ref.node) || ts3.isMethodDeclaration(ref.node) || ts3.isFunctionExpression(ref.node);
}
...
function isVariableDeclarationDeclared(node) {
  if (node.parent === void 0 || !ts3.isVariableDeclarationList(node.parent)) {
    return false;
  }
  const declList = node.parent;
  if (declList.parent === void 0 || !ts3.isVariableStatement(declList.parent)) {
    return false;
  }
  const varStmt = declList.parent;
  const modifiers = ts3.getModifiers(varStmt);
  return modifiers !== void 0 && modifiers.some((mod) => mod.kind === ts3.SyntaxKind.DeclareKeyword);
}
var EMPTY = {};
...
function getContainerNode(node) {
  ...
  while (currentNode !== void 0) {
    switch (currentNode.kind) {
      ...
      case ts4.SyntaxKind.ReturnStatement:
      case ts4.SyntaxKind.IfStatement:
      case ts4.SyntaxKind.SwitchStatement:
      case ts4.SyntaxKind.DoStatement:
      case ts4.SyntaxKind.WhileStatement:
      case ts4.SyntaxKind.ForStatement:
      case ts4.SyntaxKind.ForInStatement:
      case ts4.SyntaxKind.ForOfStatement:
      case ts4.SyntaxKind.ContinueStatement:
      case ts4.SyntaxKind.BreakStatement:
      case ts4.SyntaxKind.ThrowStatement:
...
var TraitCompiler = class {
  ...
  compile(clazz, constantPool) {
    const original = ts6.getOriginalNode(clazz);
  ...
  decoratorsFor(node) {
    const original = ts6.getOriginalNode(node);
    ...
    for (const trait of record.traits) {
      ...
      if (trait.detected.trigger !== null && ts6.isDecorator(trait.detected.trigger)) {
...
function containsErrors(diagnostics) {
  return diagnostics !== null && diagnostics.some((diag) => diag.category === ts6.DiagnosticCategory.Error);
}
...
var IvyTransformationVisitor = class extends Visitor {
  ...
  visitClassDeclaration(node) {
    const original = ts9.getOriginalNode(node, ts9.isClassDeclaration);
    ...
    const filteredDecorators = (
      ...
      maybeFilterDecorator(ts9.getDecorators(node), this.compilation.decoratorsFor(node))
    );
...
function insertFileOverviewComment(sf, fileoverview) {
  ...
  if (sf.statements.length > 0 && host !== sf.statements[0]) {
    ...
    const commentNode = ts9.factory.createNotEmittedStatement(sf);
    ts9.setSyntheticLeadingComments(commentNode, comments);
    return ts9.factory.updateSourceFile(sf, [commentNode, ...sf.statements], sf.isDeclarationFile, sf.referencedFiles, sf.typeReferenceDirectives, sf.hasNoDefaultLib, sf.libReferenceDirectives);
  }
  return sf;
}
...
function insertDebugNameIntoCallExpression(node, debugName) {
  ...
  if (existingArg !== null && (!ts10.isObjectLiteralExpression(existingArg) || existingArg.properties.some((prop) => ts10.isPropertyAssignment(prop) && ts10.isIdentifier(prop.name) && prop.name.text === "debugName"))) {
    return node;
  }
  const debugNameProperty = ts10.factory.createPropertyAssignment("debugName", ts10.factory.createStringLiteral(debugName));
  let newArgs;
  if (existingArg !== null) {
    const transformedArg = ts10.factory.createObjectLiteralExpression([
      ts10.factory.createSpreadAssignment(createNgDevModeConditional(ts10.factory.createObjectLiteralExpression([debugNameProperty]), ts10.factory.createObjectLiteralExpression())),
    ...
    if (hasNoArgs && !isRequired) {
      spreadArgs.push(ts10.factory.createIdentifier("undefined"));
    }
    spreadArgs.push(ts10.factory.createObjectLiteralExpression([debugNameProperty]));
    const spread = ts10.factory.createSpreadElement(createNgDevModeConditional(ts10.factory.createArrayLiteralExpression(spreadArgs), ts10.factory.createArrayLiteralExpression()));
    ts10.addSyntheticLeadingComment(spread, ts10.SyntaxKind.MultiLineCommentTrivia, " @ts-ignore ", true);
    newArgs = [...node.arguments, spread];
  }
  return ts10.factory.updateCallExpression(node, node.expression, node.typeArguments, newArgs);
}
function createNgDevModeConditional(devModeExpression, prodModeExpression) {
  ts10.addSyntheticLeadingComment(prodModeExpression, ts10.SyntaxKind.MultiLineCommentTrivia, " istanbul ignore next ", false);
  return ts10.factory.createParenthesizedExpression(ts10.factory.createConditionalExpression(ts10.factory.createIdentifier("ngDevMode"), void 0, devModeExpression, void 0, prodModeExpression));
}
function isVariableDeclarationCase(node) {
  ...
  return ts10.isIdentifier(expression) && isSignalFunction(expression);
}
function isPropertyAssignmentCase(node) {
  if (!ts10.isExpressionStatement(node)) {
    return false;
  }
  if (!ts10.isBinaryExpression(node.expression)) {
    return false;
  }
  const binaryExpression = node.expression;
  if (binaryExpression.operatorToken.kind !== ts10.SyntaxKind.EqualsToken) {
    return false;
  }
  if (!ts10.isCallExpression(binaryExpression.right)) {
    return false;
  }
  if (!ts10.isPropertyAccessExpression(binaryExpression.left)) {
    return false;
  }
...
function isPropertyDeclarationCase(node) {
  if (!ts10.isPropertyDeclaration(node)) {
    return false;
  }
  if (!(node.initializer && ts10.isCallExpression(node.initializer))) {
    return false;
...
function expressionIsUsingAngularImportedSymbol(program, expression) {
  ...
  if (!ts10.isImportSpecifier(importSpecifier)) {
    return false;
  }
  const namedImports = importSpecifier.parent;
  if (!ts10.isNamedImports(namedImports)) {
    return false;
  }
  const importsClause = namedImports.parent;
  if (!ts10.isImportClause(importsClause)) {
    return false;
  }
  const importDeclaration = importsClause.parent;
  if (!ts10.isImportDeclaration(importDeclaration) || !ts10.isStringLiteral(importDeclaration.moduleSpecifier)) {
    return false;
  }
...
function isRequiredSignalFunction(expression) {
  if (ts10.isPropertyAccessExpression(expression) && ts10.isIdentifier(expression.name) && ts10.isIdentifier(expression.expression)) {
...
function transformVariableDeclaration(program, node) {
  ...
  try {
    ...
    return ts10.factory.updateVariableDeclaration(node, node.name, node.exclamationToken, node.type, insertDebugNameIntoCallExpression(node.initializer, nodeText));
  } catch {
    return node;
  }
}
function transformPropertyAssignment(program, node) {
  ...
  return ts10.factory.updateExpressionStatement(node, ts10.factory.createBinaryExpression(node.expression.left, node.expression.operatorToken, insertDebugNameIntoCallExpression(node.expression.right, node.expression.left.name.text)));
}
function transformPropertyDeclaration(program, node) {
  ...
  try {
    const nodeText = node.name.getText();
    return ts10.factory.updatePropertyDeclaration(node, node.modifiers, node.name, node.questionToken, node.type, insertDebugNameIntoCallExpression(node.initializer, nodeText));
  } catch {
    return node;
  }
}
function isSignalWithObjectOnlyDefinition(callExpression) {
  ...
  const isComputationLinkedSignal = isLinkedSignal && nodeArgs[0].kind === ts10.SyntaxKind.ObjectLiteralExpression;
  ...
  ctorParams.forEach((param, idx) => {
    ...
    (param.decorators || []).filter((dec) => isCore || isAngularCore(dec)).forEach((dec) => {
      ...
      if (name === "Inject") {
        ...
        if (ts11.isStringLiteralLike(attributeName)) {
          attributeNameType = new LiteralExpr(attributeName.text);
        } else {
          attributeNameType = new WrappedNodeExpr2(ts11.factory.createKeywordTypeNode(ts11.SyntaxKind.UnknownKeyword));
        }
      } else {
...
function extractClassMetadata(clazz, reflection, isCore, annotateForClosureCompiler, angularDecoratorTransform = (dec) => dec, undecoratedMetadataExtractor = () => null) {
  ...
  for (const member of classMembers) {
    const shouldQuoteName = member.nameNode !== null && ts14.isStringLiteralLike(member.nameNode);
...
function tryParseInitializerApi(functions, expression, reflector, importTracker) {
  if (ts15.isAsExpression(expression) || ts15.isParenthesizedExpression(expression)) {
    return tryParseInitializerApi(functions, expression.expression, reflector, importTracker);
  }
  if (!ts15.isCallExpression(expression)) {
    return null;
  }
...
function parseTopLevelRequiredCall(call, functions, importTracker) {
  const node = call.expression;
  if (!ts15.isPropertyAccessExpression(node) || !ts15.isIdentifier(node.expression) || node.name.text !== "required") {
    return null;
  }
...
function parseTopLevelCallFromNamespace(call, functions, importTracker) {
  const node = call.expression;
  if (!ts15.isPropertyAccessExpression(node)) {
    return null;
  }
  let apiReference = null;
  let matchingApi = void 0;
  let isRequired = false;
  if (ts15.isIdentifier(node.expression) && ts15.isIdentifier(node.name)) {
    ...
    ts15.isPropertyAccessExpression(node.expression) && ts15.isIdentifier(node.expression.expression) && ts15.isIdentifier(node.expression.name) && node.name.text === "required"
  ) {
...
function parseAndValidateInputAndOutputOptions(optionsNode) {
  if (!ts16.isObjectLiteralExpression(optionsNode)) {
  ...
  if (options.has("alias")) {
    ...
    if (!ts16.isStringLiteralLike(aliasExpr)) {
...
function tryParseSignalQueryFromInitializer(member, reflector, importTracker) {
  ...
  if (optionsNode !== void 0 && !ts17.isObjectLiteralExpression(optionsNode)) {
...
function parseLocator(expression, reflector) {
  ...
  if (ts17.isStringLiteralLike(expression)) {
...
function parseReadOption(value) {
  if (ts17.isExpressionWithTypeArguments(value) || ts17.isParenthesizedExpression(value) || ts17.isAsExpression(value)) {
    return parseReadOption(value.expression);
  }
  if (ts17.isPropertyAccessExpression(value) && ts17.isIdentifier(value.expression) || ts17.isIdentifier(value)) {
...
function parseDescendantsOption(value) {
  if (value.kind === ts17.SyntaxKind.TrueKeyword) {
    return true;
  } else if (value.kind === ts17.SyntaxKind.FalseKeyword) {
    return false;
  }
...
function extractDecoratorQueryMetadata(exprNode, name, args, propertyName, reflector, evaluator) {
  ...
  if (args.length === 2) {
    ...
    if (!ts18.isObjectLiteralExpression(optionsExpr)) {
...
function extractHostBindings(members, evaluator, coreModule, compilationMode, hostBindingNodes, metadata) {
  ...
  if (metadata && metadata.has("host")) {
    ...
    if (ts18.isObjectLiteralExpression(hostExpression)) {
  ...
  filterToMembersWithDecorator(members, "HostBinding", coreModule).forEach(({ member, decorators }) => {
    decorators.forEach((decorator) => {
      ...
      if (ts18.isDecorator(decorator.node)) {
        const member2 = decorator.node.parent;
        hostBindingNodes.rawNodes.push(decorator.node.expression);
        if (member2 && ts18.isPropertyDeclaration(member2) && member2.name) {
  ...
  filterToMembersWithDecorator(members, "HostListener", coreModule).forEach(({ member, decorators }) => {
    decorators.forEach((decorator) => {
      ...
      if (ts18.isDecorator(decorator.node)) {
        const member2 = decorator.node.parent;
        hostBindingNodes.rawNodes.push(decorator.node.expression);
        if (member2 && ts18.isMethodDeclaration(member2) && member2.name) {
          const memberName = sourceNodeFromTs(member2.name);
          if (memberName.kind === "string" || memberName.kind === "identifier") {
            let eventName2 = null;
            let args2;
            if (decorator.args !== null && decorator.args.length > 0) {
              ...
              args2 = decorator.args.length > 1 && ts18.isArrayLiteralExpression(decorator.args[1]) ? decorator.args[1].elements.map(sourceNodeFromTs) : [];
            }
...
function sourceNodeFromTs(node) {
  ...
  if (ts18.isStringLiteralLike(node) || ts18.isIdentifier(node)) {
    ...
    return {
      kind: ts18.isIdentifier(node) ? "identifier" : "string",
      sourceSpan,
      source: node.getText(),
      text: node.text
    };
...
function extractQueriesFromDecorator(queryData, reflector, evaluator, isCore) {
  const content = [];
  const view = [];
  if (!ts18.isObjectLiteralExpression(queryData)) {
  ...
  reflectObjectLiteral(queryData).forEach((queryExpr, propertyName) => {
    ...
    if (!ts18.isNewExpression(queryExpr)) {
    ...
    const queryType = ts18.isPropertyAccessExpression(queryExpr.expression) ? queryExpr.expression.name : queryExpr.expression;
    if (!ts18.isIdentifier(queryType)) {
      throw new FatalDiagnosticError(ErrorCode.VALUE_HAS_WRONG_TYPE, queryData, "Decorator query metadata must be an instance of a query type");
    }
...
function memberMetadataFromSignalQuery(call) {
  ...
  const firstArgMeta = ts18.isStringLiteralLike(firstArg) || ts18.isCallExpression(firstArg) ? new WrappedNodeExpr5(firstArg) : (
  ...
  const entries = [
    ...
    firstArgMeta,
    new WrappedNodeExpr5(ts18.factory.createObjectLiteralExpression([
      ...call.arguments.length > 1 ? [ts18.factory.createSpreadAssignment(call.arguments[1])] : [],
      ts18.factory.createPropertyAssignment("isSignal", ts18.factory.createTrue())
    ]))
  ];
...
function parseDecoratorInputTransformFunction(clazz, classPropertyName, value, reflector, refEmitter, compilationMode, emitDeclarationOnly) {
  ...
  if (compilationMode === CompilationMode.LOCAL) {
    ...
    return {
      node: node2,
      type: new Reference(ts18.factory.createKeywordTypeNode(ts18.SyntaxKind.UnknownKeyword))
    };
  }
...
function assertEmittableInputType(type, contextFile, reflector, refEmitter) {
  (function walk(node) {
    if (ts18.isTypeReferenceNode(node) && ts18.isIdentifier(node.typeName)) {
...
function extractControlDirectiveDefinition(members) {
  ...
  if (controlCreateMember === void 0 || controlCreateMember.node === null || !ts18.isMethodDeclaration(controlCreateMember.node)) {
    return null;
  }
  const { node } = controlCreateMember;
  if (node.parameters.length === 0 || node.parameters[0].type === void 0 || !ts18.isTypeReferenceNode(node.parameters[0].type)) {
    return { passThroughInput: null };
  }
  const type = node.parameters[0].type;
  if (type.typeArguments?.length !== 1 || !ts18.isLiteralTypeNode(type.typeArguments[0]) || !ts18.isStringLiteral(type.typeArguments[0].literal)) {
    return { passThroughInput: null };
  }
...
function getHostBindingErrorNode(error, hostExpr) {
  if (ts18.isObjectLiteralExpression(hostExpr)) {
    for (const prop of hostExpr.properties) {
      if (ts18.isPropertyAssignment(prop) && ts18.isStringLiteralLike(prop.initializer) && error.msg.includes(`[${prop.initializer.text}]`)) {
...
function extractHostDirectives(rawHostDirectives, evaluator, reflector, compilationMode, forwardRefResolver, emitDeclarationOnly) {
  ...
  return resolved.map((value) => {
    ...
    if (compilationMode === CompilationMode.LOCAL && hostReference instanceof DynamicValue) {
      if (!ts18.isIdentifier(hostReference.node) && !ts18.isPropertyAccessExpression(hostReference.node)) {
      ...
      if (emitDeclarationOnly) {
        if (ts18.isIdentifier(hostReference.node)) {
...
import { compileClassDebugInfo, compileHmrInitializer, compileComponentClassMetadata, compileComponentDeclareClassMetadata, compileComponentFromMetadata, compileDeclareComponentFromMetadata, compileDeferResolverFunction, ConstantPool as ConstantPool2, CssSelector as CssSelector2, DomElementSchemaRegistry as DomElementSchemaRegistry2, ExternalExpr as ExternalExpr7, FactoryTarget as FactoryTarget3, makeBindingParser as makeBindingParser2, outputAst as o4, R3TargetBinder as R3TargetBinder2, R3TemplateDependencyKind, SelectorMatcher, ViewEncapsulation as ViewEncapsulation2, SelectorlessMatcher, MatchSource as MatchSource2 } from "@angular/compiler";
...
import { compileClassMetadata, compileDeclareClassMetadata, compileDeclareDirectiveFromMetadata, compileDirectiveFromMetadata, FactoryTarget, makeBindingParser, MatchSource, R3TargetBinder, WrappedNodeExpr as WrappedNodeExpr8 } from "@angular/compiler";
...
import { CssSelector, DomElementSchemaRegistry, ExternalExpr as ExternalExpr5, WrappedNodeExpr as WrappedNodeExpr7 } from "@angular/compiler";
...
var CompletionEngine = class {
  ...
  getLiteralCompletionLocation(expr) {
    ...
    if (expr instanceof TmplAstTextAttribute) {
      ...
      tsExpr = findFirstMatchingNode(this.tcb, {
        filter: (n2) => ts22.isStringLiteral(n2) || ts22.isNumericLiteral(n2),
        withSpan: expr.sourceSpan
      });
    }
    if (tsExpr === null) {
...
import { AbsoluteSourceSpan, ExternalExpr as ExternalExpr4, TransplantedType, WrappedNodeExpr as WrappedNodeExpr6, ClassPropertyMapping as ClassPropertyMapping2 } from "@angular/compiler";
...
function adaptTypeCheckBlockMetadata(ref, meta, env, genericContextBehavior) {
  ...
  const convertDir = (dir) => {
    ...
    const tcbDir = {
      ...
      hasNgFieldDirective: ts23.isClassDeclaration(dir.ref.node) && dir.ref.node.members.some((member) => ts23.isPropertyDeclaration(member) && ts23.isComputedPropertyName(member.name) && ts23.isIdentifier(member.name.expression) && member.name.expression.text === "\u0275NgFieldDirective"),
...
function extractReferenceMetadata(ref, env) {
  ...
  if (emitted.kind === ReferenceEmitKind.Success) {
    if (emitted.expression instanceof ExternalExpr4) {
...
import { AbsoluteSourceSpan as AbsoluteSourceSpan2, BindingType, ParseSourceSpan as ParseSourceSpan2, TmplAstBoundAttribute, TmplAstBoundEvent, TmplAstComponent, TmplAstDirective, TmplAstElement } from "@angular/compiler";
...
var OutOfBandDiagnosticRecorderImpl = class {
  ...
  missingReferenceTarget(id, ref) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, mapping, ref.valueSpan || ref.sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.MISSING_REFERENCE_TARGET), errorMsg));
  }
...
To fix this, import the "${suggestedClassName}" class from "${suggestedImport}" and add it to the "imports" array of the module declaring the component.`;
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, mapping, sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.MISSING_PIPE), errorMsg));
  ...
  deferredPipeUsedEagerly(id, ast) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, mapping, sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.DEFERRED_PIPE_USED_EAGERLY), errorMsg));
    this.recordedPipes.add(ast);
  }
  deferredComponentUsedEagerly(id, element) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, mapping, sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.DEFERRED_DIRECTIVE_USED_EAGERLY), errorMsg));
  }
  duplicateTemplateVar(id, variable, firstDecl) {
    const mapping = this.resolver.getTemplateSourceMapping(id);
    const errorMsg = `Cannot redeclare variable '${variable.name}' as it was previously declared elsewhere for the same template.`;
    this._diagnostics.push(makeTemplateDiagnostic(id, mapping, variable.sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.DUPLICATE_VARIABLE_DECLARATION), errorMsg, [
      {
...
Consider enabling the 'strictTemplates' option in your tsconfig.json for better type inference within this template.`;
    this._diagnostics.push(makeTemplateDiagnostic(id, mapping, diagnosticVar.keySpan, ts24.DiagnosticCategory.Suggestion, ngErrorCode(ErrorCode.SUGGEST_SUBOPTIMAL_TYPE_INFERENCE), message));
  }
 ...
 Are you missing an output declaration called '${output.name}'?`;
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, mapping, input.keySpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.SPLIT_TWO_WAY_BINDING), errorMsg, relatedMessages));
  }
  missingRequiredInputs(id, element, directiveName, isComponent, inputAliases) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), this.getTagNameSpan(element), ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.MISSING_REQUIRED_INPUTS), message));
  }
  illegalForLoopTrackAccess(id, block, access) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.ILLEGAL_FOR_LOOP_TRACK_ACCESS), message));
  }
...
Deferred blocks can only access triggers in same view, a parent embedded view or the root view of the @placeholder block.`;
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), trigger.sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.INACCESSIBLE_DEFERRED_TRIGGER_ELEMENT), message));
  }
...
`,
  ...
  illegalWriteToLetDeclaration(id, node, target) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.ILLEGAL_LET_WRITE), `Cannot assign to @let declaration '${target.name}'.`));
  }
  letUsedBeforeDefinition(id, node, target) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.LET_USED_BEFORE_DEFINITION), `Cannot read @let declaration '${target.name}' before it has been defined.`));
  }
  conflictingDeclaration(id, decl) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, mapping, decl.sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.CONFLICTING_LET_DECLARATION), errorMsg));
  }
  ...
  unclaimedDirectiveBinding(id, directive, node) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), node.keySpan || node.sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.UNCLAIMED_DIRECTIVE_BINDING), errorMsg));
  }
  deferImplicitTriggerMissingPlaceholder(id, trigger) {
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), trigger.sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.DEFER_IMPLICIT_TRIGGER_MISSING_PLACEHOLDER), "Trigger with no target can only be placed on an @defer that has a @placeholder block"));
  }
  deferImplicitTriggerInvalidPlaceholder(id, trigger) {
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), trigger.sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.DEFER_IMPLICIT_TRIGGER_INVALID_PLACEHOLDER), "Trigger with no target can only be placed on an @defer that has a @placeholder block with exactly one root element node"));
  }
  formFieldUnsupportedBinding(id, node) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), node.sourceSpan, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.FORM_FIELD_UNSUPPORTED_BINDING), message));
  }
  multipleMatchingComponents(id, element, componentNames) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), span, ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.MULTIPLE_MATCHING_COMPONENTS), `Multiple components match node with tagname ${element.name}: ${names}.`));
  }
  conflictingHostDirectiveBinding(id, node, directiveName, kind, classPropertyName, aliases) {
    ...
    this._diagnostics.push(makeTemplateDiagnostic(id, this.resolver.getTemplateSourceMapping(id), this.getTagNameSpan(node), ts24.DiagnosticCategory.Error, ngErrorCode(ErrorCode.CONFLICTING_HOST_DIRECTIVE_BINDING), message));
  }
  getTagNameSpan(node) {
    let span;
    let name;
...
function translateCategory(category) {
  switch (category) {
    case OutOfBadDiagnosticCategory.Error:
      return ts24.DiagnosticCategory.Error;
    case OutOfBadDiagnosticCategory.Warning:
      return ts24.DiagnosticCategory.Warning;
  }
}
...
var TypeCheckFile = class extends Environment {
  ...
  constructor(fileName, config, refEmitter, reflector, compilerHost) {
    ...
    }), refEmitter, reflector, ts26.createSourceFile(compilerHost.getCanonicalFileName(fileName), "", ts26.ScriptTarget.Latest, true));
  ...
  render() {
    ...
    if (newImports !== void 0) {
      source += newImports.map((i) => printer.printNode(ts26.EmitHint.Unspecified, i, this.contextFile)).join("\n");
    }
    source += "\n";
  ...
  transform(sf) {
    ...
    if (newImports.has(sf.fileName)) {
      newImports.get(sf.fileName).forEach((newImport) => {
        updates.push({
          pos: 0,
          text: printer.printNode(ts27.EmitHint.Unspecified, newImport, sf)
        });
      });
    }
    for (const [oldBindings, newBindings] of updatedImports.entries()) {
      ...
      updates.push({
        ...
        text: printer.printNode(ts27.EmitHint.Unspecified, newBindings, sf)
      });
    }
  ...
  pendingShimForClass(node) {
    ...
    if (!fileData.shimData.has(shimPath)) {
      fileData.shimData.set(shimPath, {
        ...
        oobRecorder: new OutOfBandDiagnosticRecorderImpl(fileData.sourceManager, (name) => this.compilerHost.getSourceFile(name, ts27.ScriptTarget.Latest)),
...
var TemplateTypeCheckerImpl = class {
  ...
  getTsSymbolOfSymbol(symbol) {
    ...
    } else if (ts28.isCallExpression(node)) {
      tsSymbol = typeChecker.getSymbolAtLocation(node.expression);
    } else if (ts28.isElementAccessExpression(node) && ts28.isStringLiteral(node.argumentExpression)) {
    ...
    if (tsSymbol === void 0 && ts28.isIdentifier(node) && node.text.startsWith("_t")) {
      let type = typeChecker.getTypeAtLocation(node);
      tsSymbol = type.aliasSymbol ?? type.symbol;
    }
  ...
  emit(kind, refTo, inContext) {
    ...
    } else if (emitted instanceof ExternalExpr5 && emitted.value.moduleName !== null && emitted.value.name !== null) {
      return {
        kind,
...
function getTheElementTagDeprecatedSuggestionDiagnostics(shimPath, program, fileRecord, diags, templateTypeChecker) {
  ...
  for (const tsDiag of diags) {
    ...
    if (decl === void 0 || !ts28.isClassDeclaration(decl)) {
      continue;
    }
  ...
  for (const directive of directiveNodesInTcb) {
    ...
    if (!ts28.isClassDeclaration(decl)) {
      continue;
    }
...
var DirectiveDecoratorHandler = class {
  ...
  typeCheck(ctx, node, meta) {
    ...
    if (!ts29.isClassDeclaration(node) || meta.isPoisoned && !this.usePoisonedData) {
      return;
    }
    const ref = new Reference(node);
...
import { compileClassMetadata as compileClassMetadata2, compileDeclareClassMetadata as compileDeclareClassMetadata2, compileDeclareInjectorFromMetadata, compileDeclareNgModuleFromMetadata, compileInjector, compileNgModule, ExternalExpr as ExternalExpr6, FactoryTarget as FactoryTarget2, FunctionExpr, InvokeFunctionExpr, LiteralArrayExpr as LiteralArrayExpr3, R3Identifiers as R3Identifiers2, R3NgModuleMetadataKind, R3SelectorScopeMode, ReturnStatement, WrappedNodeExpr as WrappedNodeExpr9 } from "@angular/compiler";
...
function createModuleWithProvidersResolver(reflector, isCore) {
  function _reflectModuleFromTypeParam(type, node) {
    if (!ts30.isTypeReferenceNode(type)) {
      return null;
    }
    const typeName = type && (ts30.isIdentifier(type.typeName) && type.typeName || ts30.isQualifiedName(type.typeName) && type.typeName.right) || null;
    if (typeName === null) {
      return null;
    }
    ...
    if (type.typeArguments === void 0 || type.typeArguments.length !== 1) {
      const parent = ts30.isMethodDeclaration(node) && ts30.isClassDeclaration(node.parent) ? node.parent : null;
  ...
  function _reflectModuleFromLiteralType(type) {
    if (!ts30.isIntersectionTypeNode(type)) {
      return null;
    }
    for (const t of type.types) {
      if (ts30.isTypeLiteralNode(t)) {
        for (const m of t.members) {
          const ngModuleType = ts30.isPropertySignature(m) && ts30.isIdentifier(m.name) && m.name.text === "ngModule" && m.type || null;
          let ngModuleExpression = null;
          if (ngModuleType !== null && ts30.isTypeQueryNode(ngModuleType)) {
...
var NgModuleDecoratorHandler = class {
  ...
  analyze(node, decorator) {
    ...
    const meta = decorator.args.length === 1 ? unwrapExpression(decorator.args[0]) : ts31.factory.createObjectLiteralExpression([]);
    ...
    if (ngModule.has("id")) {
      ...
      if (!isModuleIdExpression(idExpr)) {
        ...
        diag.category = ts31.DiagnosticCategory.Warning;
    ...
    if (rawProviders !== null && (!ts31.isArrayLiteralExpression(rawProviders) || rawProviders.elements.length > 0)) {
    ...
    if (!allowUnresolvedReferences && ngModule.has("imports")) {
      ...
      if (ts31.isArrayLiteralExpression(rawImports2)) {
    ...
    if (allowUnresolvedReferences) {
      for (const exp of [rawImports, rawExports]) {
        ...
        if (ts31.isArrayLiteralExpression(exp)) {
          if (exp.elements) {
  ...
  appendRemoteScopingStatements(ngModuleStatements, node, declarations, remoteScopesMayRequireCycleProtection) {
    ...
    for (const decl of declarations) {
      ...
      if (remoteScope !== null) {
        ...
        const setComponentScope = new ExternalExpr6(R3Identifiers2.setComponentScope);
...
function isModuleIdExpression(expr) {
  return ts31.isPropertyAccessExpression(expr) && ts31.isIdentifier(expr.expression) && expr.expression.text === "module" && expr.name.text === "id";
}
...
function validateAndFlattenComponentImports(imports, expr, isDeferred) {
  ...
  for (let i = 0; i < imports.length; i++) {
    const ref = imports[i];
    let refExpr = expr;
    if (ts32.isArrayLiteralExpression(expr) && expr.elements.length === imports.length && !expr.elements.some(ts32.isSpreadAssignment)) {
...
function extractHmrDependencies(node, definition, factory, deferBlockMetadata, classMetadata, debugInfo, reflection, evaluator) {
  const name = ts33.isClassDeclaration(node) && node.name ? node.name.text : null;
  const visitor = new PotentialTopLevelReadsVisitor();
  const sourceFile = ts33.getOriginalNode(node).getSourceFile();
...
function getTopLevelDeclarationNames(sourceFile) {
  ...
  for (const node of sourceFile.statements) {
    if (ts33.isClassDeclaration(node) || ts33.isFunctionDeclaration(node) || ts33.isEnumDeclaration(node)) {
      if (node.name) {
        results.add(node.name.text);
      }
      continue;
    }
    if (ts33.isVariableStatement(node)) {
      for (const decl of node.declarationList.declarations) {
        trackBindingName(decl.name, results);
      }
      continue;
    }
    if (ts33.isImportDeclaration(node) && node.importClause) {
      const importClause = node.importClause;
      if (importClause.phaseModifier === ts33.SyntaxKind.TypeKeyword) {
        continue;
      }
      ...
      if (importClause.namedBindings) {
        ...
        if (ts33.isNamespaceImport(namedBindings)) {
...
function trackBindingName(node, results) {
  if (ts33.isIdentifier(node)) {
    results.add(node.text);
  } else {
    for (const el of node.elements) {
      if (!ts33.isOmittedExpression(el)) {
...
var PotentialTopLevelReadsVisitor = class extends o2.RecursiveAstVisitor {
  ...
  addAllTopLevelIdentifiers = (node) => {
    if (ts33.isIdentifier(node) && this.isTopLevelIdentifierReference(node)) {
      this.allReads.add(node);
    } else {
      ts33.forEachChild(node, this.addAllTopLevelIdentifiers);
    }
  };
  /**
  ...
  isTopLevelIdentifierReference(identifier) {
    ...
    if (ts33.isParenthesizedExpression(parent) && parent.expression === node) {
      while (parent && ts33.isParenthesizedExpression(parent)) {
        node = parent;
        parent = parent.parent;
      }
    }
    if (ts33.isSourceFile(parent)) {
      return true;
    }
    if (ts33.isCallExpression(parent)) {
      return parent.expression === node || parent.arguments.includes(node);
    }
    if (ts33.isExpressionStatement(parent) || ts33.isPropertyAccessExpression(parent) || ts33.isComputedPropertyName(parent) || ts33.isTemplateSpan(parent) || ts33.isSpreadAssignment(parent) || ts33.isSpreadElement(parent) || ts33.isAwaitExpression(parent) || ts33.isNonNullExpression(parent) || ts33.isIfStatement(parent) || ts33.isDoStatement(parent) || ts33.isWhileStatement(parent) || ts33.isSwitchStatement(parent) || ts33.isCaseClause(parent) || ts33.isThrowStatement(parent) || ts33.isNewExpression(parent) || ts33.isExpressionWithTypeArguments(parent)) {
      return parent.expression === node;
    }
    if (ts33.isArrayLiteralExpression(parent)) {
      return parent.elements.includes(node);
    }
    if (ts33.isPropertyAssignment(parent) || ts33.isParameter(parent) || ts33.isBindingElement(parent) || ts33.isPropertyDeclaration(parent) || ts33.isEnumMember(parent)) {
      return parent.initializer === node;
    }
    if (ts33.isVariableDeclaration(parent)) {
      return parent.name === node || parent.initializer === node;
    }
    if (ts33.isClassDeclaration(parent) || ts33.isFunctionDeclaration(parent) || ts33.isShorthandPropertyAssignment(parent)) {
      return parent.name === node;
    }
    if (ts33.isElementAccessExpression(parent)) {
      return parent.expression === node || parent.argumentExpression === node;
    }
    if (ts33.isBinaryExpression(parent)) {
      return parent.left === node || parent.right === node;
    }
    if (ts33.isForInStatement(parent) || ts33.isForOfStatement(parent)) {
      return parent.expression === node || parent.initializer === node;
    }
    if (ts33.isForStatement(parent)) {
      return parent.condition === node || parent.initializer === node || parent.incrementor === node;
    }
    if (ts33.isArrowFunction(parent)) {
      return parent.body === node;
    }
    if (ts33.isImportSpecifier(parent) || ts33.isExportSpecifier(parent)) {
      return (parent.propertyName || parent.name) === node;
    }
    if (ts33.isConditionalExpression(parent)) {
...
function isConstEnumReference(node, reflection) {
  ...
  if (!parent || !ts33.isPropertyAccessExpression(parent) || parent.expression !== node || !ts33.isIdentifier(parent.name)) {
    return false;
  }
  const declaration = reflection.getDeclarationOfIdentifier(node);
  return declaration !== null && ts33.isEnumDeclaration(declaration.node) && !!declaration.node.modifiers?.some((m) => m.kind === ts33.SyntaxKind.ConstKeyword);
}
...
function extractHmrMetatadata(clazz, reflection, evaluator, compilerHost, rootDirs, definition, factory, deferBlockMetadata, classMetadata, debugInfo) {
  ...
  const sourceFile = ts34.getOriginalNode(clazz).getSourceFile();
...
function getHmrUpdateDeclaration(compilationResults, constantStatements, meta, declaration) {
  ...
  const sourceFile = ts35.getOriginalNode(declaration).getSourceFile();
  const node = translateStatement(sourceFile, callback, importManager);
  return ts35.factory.updateFunctionDeclaration(node, [
    ts35.factory.createToken(ts35.SyntaxKind.ExportKeyword),
    ts35.factory.createToken(ts35.SyntaxKind.DefaultKeyword)
...
import { BindingPipe, CombinedRecursiveAstVisitor, tmplAstVisitAll, BindingPipeType } from "@angular/compiler";
...
var ComponentDecoratorHandler = class {
  ...
  analyze(node, decorator) {
    ...
    const relativeTemplatePath = getProjectRelativePath(templateResource.path ?? ts36.getOriginalNode(node).getSourceFile().fileName, this.rootDirs, this.compilerHost);
    ...
    for (const styleUrl of styleUrls) {
      try {
        ...
        if (styleUrl.source === 2 && ts36.isStringLiteralLike(styleUrl.expression)) {
          styleResources.add({
  ...
  resolveComponentDependencies(node, context, analysis, scope, metadata, diagnostics) {
    ...
    if (explicitlyDeferredDependencies !== null && explicitlyDeferredDependencies.length > 0) {
      ...
      const deferBlockMatcher = new SelectorMatcher();
      for (const dep of allDependencies) {
        if (dep.kind === MetaKind.Pipe && dep.name !== null) {
          pipes.set(dep.name, dep);
        } else if (dep.kind === MetaKind.Directive && dep.selector !== null) {
          deferBlockMatcher.addSelectables(CssSelector2.parse(dep.selector), [dep]);
        }
      }
    ...
    if (!cycleDetected) {
      ...
      if (this.compilationMode === CompilationMode.LOCAL && this.localCompilationExtraImportsTracker !== null) {
        for (const { type } of eagerDeclarations) {
          if (type instanceof ExternalExpr7 && type.value.moduleName) {
  ...
  collectExplicitlyDeferredSymbols(rawDeferredImports) {
    ...
    if (!ts36.isArrayLiteralExpression(rawDeferredImports)) {
      return deferredTypes;
    }
    for (const element of rawDeferredImports.elements) {
      const node = tryUnwrapForwardRef(element, this.reflector) || element;
      if (!ts36.isIdentifier(node)) {
        continue;
      }
  ...
  resolveDeferBlocks(componentClassDecl, scope, deferBlocks, deferrableDecls, resolutionData, analysisData, eagerlyUsedDecls) {
    ...
    if (analysisData.meta.isStandalone) {
      if (analysisData.rawImports !== null && ts36.isArrayLiteralExpression(analysisData.rawImports)) {
      ...
      if (analysisData.rawDeferredImports !== null && ts36.isArrayLiteralExpression(analysisData.rawDeferredImports)) {
...
function extractInjectableMetadata(clazz, decorator, reflector) {
  ...
  if (decorator.args.length === 0) {
    ...
    if (!ts37.isObjectLiteralExpression(metaNode)) {
    ...
    if ((meta.has("useClass") || meta.has("useFactory")) && meta.has("deps")) {
      ...
      if (!ts37.isArrayLiteralExpression(depsExpr)) {
...
function getDep(dep, reflector) {
  ...
  if (ts37.isArrayLiteralExpression(dep)) {
    dep.elements.forEach((el) => {
      let isDecorator = false;
      if (ts37.isIdentifier(el)) {
        isDecorator = maybeUpdateDecorator(el, reflector);
      } else if (ts37.isNewExpression(el) && ts37.isIdentifier(el.expression)) {
...
var PipeDecoratorHandler = class {
  ...
  analyze(clazz, decorator) {
    ...
    ts38.isNonNullExpression(decorator.args[0]) && decorator.args[0].expression.kind === ts38.SyntaxKind.NullKeyword ? null : unwrapExpression(decorator.args[0]);
    let pipeName = null;
...
function createSyntheticAngularCoreDecoratorAccess(factory, importManager, ngClassDecorator, sourceFile, decoratorName) {
  const classDecoratorIdentifier = ts39.isIdentifier(ngClassDecorator.identifier) ? ngClassDecorator.identifier : ngClassDecorator.identifier.expression;
  return factory.createPropertyAccessExpression(
    ...
    ts39.setOriginalNode(factory.createIdentifier(decoratorName), classDecoratorIdentifier)
  );
}
function castAsAny(factory, expr) {
  return factory.createAsExpression(expr, factory.createKeywordTypeNode(ts39.SyntaxKind.AnyKeyword));
}
...
var signalModelTransform = (member, sourceFile, host, factory, importTracker, importManager, classDecorator, isCore) => {
  ...
  const inputDecorator = createDecorator(
    ...
    // transform is used directly as a pre-compilation step, the decorator should
    // not fail. It is already validated now due to us parsing the input metadata.
    factory.createAsExpression(inputConfig, factory.createKeywordTypeNode(ts40.SyntaxKind.AnyKeyword)),
    classDecorator,
    factory,
    sourceFile,
    importManager
...
function getInitializerApiJitTransform(host, importTracker, isCore, shouldTransformClass) {
  return (ctx) => {
    return (sourceFile) => {
      const importManager = new ImportManager();
      sourceFile = ts41.visitNode(sourceFile, createTransformVisitor(ctx, host, importManager, importTracker, isCore, shouldTransformClass), ts41.isSourceFile);
...
function createTransformVisitor(ctx, host, importManager, importTracker, isCore, shouldTransformClass) {
  const visitor = (node) => {
    if (ts41.isClassDeclaration(node) && node.name !== void 0) {
      const originalNode = ts41.getOriginalNode(node, ts41.isClassDeclaration);
      ...
      if (angularDecorator !== void 0 && (shouldTransformClass === void 0 || shouldTransformClass(node))) {
        ...
        const members = node.members.map((memberNode) => {
          if (!ts41.isPropertyDeclaration(memberNode)) {
            return memberNode;
          }
    ...
    return ts41.visitEachChild(node, visitor, ctx);
  };
  return visitor;
}
