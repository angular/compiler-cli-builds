...
import { compileClassDebugInfo, compileComponentClassMetadata, compileComponentFromMetadata, compileDeclareClassMetadata as compileDeclareClassMetadata3, compileDeclareComponentFromMetadata, CssSelector as CssSelector2, DEFAULT_INTERPOLATION_CONFIG as DEFAULT_INTERPOLATION_CONFIG2, DomElementSchemaRegistry, ExternalExpr as ExternalExpr6, FactoryTarget as FactoryTarget3, makeBindingParser as makeBindingParser2, outputAst as o2, R3TargetBinder, R3TemplateDependencyKind, SelectorMatcher as SelectorMatcher2, ViewEncapsulation as ViewEncapsulation2 } from "@angular/compiler";
...
var ComponentDecoratorHandler = class {
  ...
  resolve(node, analysis, symbol) {
    ...
    if (this.compilationMode === CompilationMode.LOCAL) {
      data = {
        ...
        deferrableTypes: (_a = analysis.explicitlyDeferredTypes) != null ? _a : /* @__PURE__ */ new Map()
      };
      ...
      data = {
        ...
        deferBlockDependencies: /* @__PURE__ */ new Map(),
  ...
  collectDeferredSymbols(resolution) {
    ...
    for (const [_, deps] of resolution.deferBlockDependencies) {
      for (const deferBlockDep of deps) {
        const importDecl = (_a = resolution.deferrableDeclToImportDecl.get(deferBlockDep.type.node)) != null ? _a : null;
  ...
  compileDeferBlocks(resolution) {
    if (resolution.deferBlockDepsEmitMode === 0) {
      if (!resolution.deferBlockDependencies) {
        throw new Error("Internal error: deferBlockDependencies must be present when compiling in PerBlock mode");
      }
      ...
      for (const [block, dependencies] of resolution.deferBlockDependencies) {
        ...
        for (const dep of dependencies) {
          if (dep.isDeferrable) {
            const innerFn = o2.arrowFn(
              [new o2.FnParam("m", o2.DYNAMIC_TYPE)],
              o2.variable("m").prop(dep.isDefaultImport ? "default" : dep.symbolName)
            );
            const importExpr = new o2.DynamicImportExpr(dep.importPath).prop("then").callFn([innerFn]);
            depExpressions.push(importExpr);
          } else {
            depExpressions.push(o2.variable(dep.symbolName));
          }
        }
        blocks.set(block, depExpressions.length === 0 ? null : o2.arrowFn([], o2.literalArr(depExpressions)));
      }
      return { mode: 0, blocks };
    }
    if (resolution.deferBlockDepsEmitMode === 1) {
      if (!resolution.deferBlockDependencies || !resolution.deferrableTypes) {
        throw new Error("Internal error: deferBlockDependencies and deferrableTypes must be present in PerComponent mode");
      }
      const depExpressions = [];
      for (const [symbolName, { importPath, isDefaultImport: isDefaultImport2 }] of resolution.deferrableTypes) {
        const innerFn = o2.arrowFn([new o2.FnParam("m", o2.DYNAMIC_TYPE)], o2.variable("m").prop(isDefaultImport2 ? "default" : symbolName));
        const importExpr = new o2.DynamicImportExpr(importPath).prop("then").callFn([innerFn]);
        depExpressions.push(importExpr);
      }
      return {
        mode: 1,
        dependenciesFn: depExpressions.length === 0 ? null : o2.arrowFn([], o2.literalArr(depExpressions))
      };
    }
 ...
 * found in the LICENSE file at https://angular.io/license
 */
//# sourceMappingURL=chunk-BRZFE5TW.js.map
