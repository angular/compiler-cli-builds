...
(function (factory) {
    ...
    function getImportsOfUmdModule(umdModule) {
        var imports = [];
        var cjsFactoryCall = umdModule.factoryCalls.cjsCallForImports;
        ...
        var depStartIndex = cjsFactoryCall.arguments.findIndex(function (arg) { return commonjs_umd_utils_1.isRequireCall(arg); });
        if (depStartIndex !== -1) {
            for (var i = depStartIndex; i < umdModule.factoryFn.parameters.length; i++) {
                imports.push({
    ...
    function getRequiredModulePath(cjsFactoryCall, paramIndex) {
        var requireCall = cjsFactoryCall.arguments[paramIndex];
        if (requireCall !== undefined && !commonjs_umd_utils_1.isRequireCall(requireCall)) {
            throw new Error("Argument at index " + paramIndex + " of UMD factory call is not a `require` call with a " +
